...
const CardFeature = ({ title, category, imageUrl }: CardFeatureProps) => {
      ...
      >
        <Button type="link" href="/game/sdasdk" className="rounded-xl">
          <div
            className={`${
              hoverState && "scale-90"
            } duration-500 transition-all ease-in-out overflow-hidden rounded-2xl`}
          >
            <Image type="imageFeature" url={imageUrl} alt="image thumbnail" />
          </div>
          <div
            className={`${
              hoverState ? "opacity-100" : "opacity-5"
            } duration-300 transition-opacity ease-in-out absolute top-0 p-2 overflow-hidden w-full h-full`}
          >
            <div className="flex flex-col justify-end items-end backdrop-blur-lg bg-white/5 h-full pb-10 px-5 rounded-2xl">
              <svg
                width="54"
                height="36"
                viewBox="0 0 54 36"
                ...
                className={`${
                  hoverState ? "translate-y-0" : "-translate-y-10"
              ...
              <div
                className={`${
                  hoverState ? "translate-y-0" : "translate-y-10"
                } mx-auto transition-transform duration-300 mt-10`}
              >
